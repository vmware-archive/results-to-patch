...
function indexOfStory(stories, id) {
  for (var index in stories) {
    var story = stories[index];
    if (story.id === id) return index;
  }
  return -1;
}
...
function moveStory(state, action) {
  var index = indexOfStory(state.stories, action.id);
  ...
  if (action.before_id) {
    toIndex = indexOfStory(stories, action.before_id);
  }
  else if (action.after_id) {
    toIndex = indexOfStory(stories, action.after_id) + 1;
  }
