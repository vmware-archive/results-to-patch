...
// var stateChangePositionFinder = {
//   'feature | unscheduled => scheduled': bottomOfScheduled,
//   'feature | unscheduled => started':   bottomOfStarted,
//   'feature | scheduled => started':     bottomOfStarted,
// };


var AcceptedBucket = Bucket.extend({
  criteria: [
    'feature:accepted',
    'bug:accepted',
    'chore:accepted',
...
var ActiveBucket = Bucket.extend({
  criteria: [
    'feature:rejected',
    'feature:delivered',
    'feature:finished',
...
Project.prototype.setStoryAttr = function(id, attr, value) {
  ...
  var order = [
    ['feature:accepted', 'bug:accepted', 'chore:accepted', 'release:accepted'],
    ['feature:started',  'bug:started',  'chore:started',  'release:accepted'],
    'feature:started'
  ];
  ...
  if (attr === 'current_state') {
    ...
    var moreAcceptedBuckets = {
      'release|unscheduled':  UNSCHEDUED,
      'release|scheduled':    SCHEDULED,
      'release|planned':      PLANNED,
      'release|accepted':     ACCEPTED,
      'feature|unscheduled':  UNSCHEDUED,
      'feature|scheduled':    SCHEDULED,
      'feature|planned':      PLANNED,
      'feature|started':      ACTIVE,
      'feature|finished':     ACTIVE,
      'feature|delivered':    ACTIVE,
      'feature|accepted':     ACCEPTED,
    };

    while (!newPlacementIsValid) {

    }
    // if (newPlacementIsValid) {

    // } else {

    // }
    var oldState = this.get(this.pathOfStory(id) + '/current_state');
    var direction ['accepted', 'started']
    if ()
  }
...
Project.prototype.deleteStoryFollower = function(storyId, personId) {
  this.opRemove(this.pathOfStoryFollower(storyId, personId));
};

Project.prototype.deleteEpicFollower = function(epicId, personId) {
  this.opRemove(this.pathOfEpicFollower(epicId, personId));
};
...
Project.prototype.pathOfStoryFollower = function(storyId, personId) {
  var storyIndex = this.indexOfStory(storyId);
  var story = this.get(paths.story(storyIndex));
  return paths.storyFollower(storyIndex, story.follower_ids.indexOf(personId));
}

Project.prototype.pathOfEpicFollower = function(epicId, personId) {
  var epicIndex = this.indexOfEpic(epicId);
  var epic = this.get(paths.epic(epicIndex));
  return paths.epicFollower(epicIndex, epic.follower_ids.indexOf(personId));
}
